	...
	for {
		...
		for !timeout && !done {
			...
			case <-time.After(s.Cfg.ReqTimeout()):
				if lg != nil {
					lg.Warn("timed out waiting for read index response (local node might have slow network)", zap.Duration("timeout", s.Cfg.ReqTimeout()), zap.Uint64("sent-request-id", id1))
				} else {
